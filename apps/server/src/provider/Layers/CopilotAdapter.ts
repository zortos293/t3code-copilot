...
): Extract<ModelSelection, { provider: "copilot" }> | undefined {
  return modelSelection?.provider === PROVIDER ? modelSelection : undefined;
}

function getCopilotReasoningEffort(modelSelection: ModelSelection | undefined) {
  const reasoningEffort = normalizeString(getCopilotModelSelection(modelSelection)?.options?.reasoningEffort);
  ...
  Effect.gen(function* () {
    ...
    const startSession: CopilotAdapterShape["startSession"] = (input) =>
      Effect.gen(function* () {
        ...
        const copilotModelSelection = getCopilotModelSelection(input.modelSelection);
        const selectedModel = copilotModelSelection?.model;
        if (existing) {
          return {
            provider: PROVIDER,
            status: "ready",
        ...
        const cliPath =
          normalizeCopilotCliPathOverride(copilotSettings.cliPath) ??
        ...
        const reasoningEffort = getCopilotReasoningEffort(input.modelSelection);
    ...
    const sendTurn: CopilotAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        const copilotModelSelection = getCopilotModelSelection(input.modelSelection);
        const explicitReasoningEffort = getCopilotReasoningEffort(input.modelSelection);
        const nextModel = copilotModelSelection?.model ?? record.model;
        const nextReasoningEffort =
          explicitReasoningEffort !== undefined
            ? explicitReasoningEffort
            : copilotModelSelection?.model && copilotModelSelection.model !== record.model
              ? undefined
