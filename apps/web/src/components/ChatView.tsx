...
import { resolveSubagentIdentity } from "../subagent";
import {
...
const ToolWorkEntryRow = memo(function ToolWorkEntryRow(props: {
  ...
  timestampFormat: TimestampFormat;
  onOpenSubagent?: (activityId: string) => void;
}) {
  const { workEntry, workEntryIndex, timestampFormat, onOpenSubagent } = props;
  ...
  const isSubagentEntry = Boolean(workEntry.subagentActivityId);
  const subagentIdentity = isSubagentEntry
    ? resolveSubagentIdentity(workEntry.subagentActivityId!, workEntryIndex)
    : null;
  ...
  const summaryRow = (
    ...
    >
      ...
      {subagentIdentity ? (
        <span
          className={`flex size-5 shrink-0 items-center justify-center rounded-full border ${subagentIdentity.color.bg} ${!hasExpandedDetails ? "ml-5" : ""}`}
        >
          <span className={`h-2 w-2 rounded-full ${subagentIdentity.color.dot}`} />
        </span>
      ) : (
        <span
          className={cn(
      ...
      <div className="min-w-0 flex-1 overflow-hidden">
        ...
        >
          <span
            className={cn(
              subagentIdentity ? subagentIdentity.color.text : "text-foreground/80",
              workToneClass(workEntry.tone),
            )}
          >
            {subagentIdentity ? subagentIdentity.name : heading}
          </span>
      ...
      {isSubagentEntry && onOpenSubagent && (
        <button
          type="button"
          className="shrink-0 rounded px-1.5 py-0.5 text-[9px] text-muted-foreground/60 transition-colors hover:bg-accent hover:text-foreground/80"
          onClick={(event) => {
            event.stopPropagation();
            onOpenSubagent(workEntry.subagentActivityId!);
          }}
        >
          Open →
        </button>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 transition-[opacity,translate] duration-200">
        {subagentIdentity ? (
          <span
            className={`flex size-5 shrink-0 items-center justify-center rounded-full border ${subagentIdentity.color.bg}`}
          >
            <span className={`h-2 w-2 rounded-full ${subagentIdentity.color.dot}`} />
          </span>
        ) : (
        ...
        {isSubagentEntry && onOpenSubagent && (
          <button
            type="button"
            className="shrink-0 rounded px-1.5 py-0.5 text-[9px] text-muted-foreground/60 transition-colors hover:bg-accent hover:text-foreground/80"
            onClick={() => onOpenSubagent(workEntry.subagentActivityId!)}
          >
            Open →
          </button>
        )}
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const requestedSubagentActivityId = rawSearch.subagentActivityId;
  ...
  const activeSubagentActivityId =
    requestedSubagentActivityId &&
    threadActivities.some((activity) => activity.id === requestedSubagentActivityId)
      ? requestedSubagentActivityId
      : undefined;
  const workLogEntries = useMemo(
    () => deriveVisibleThreadWorkLogEntries(threadActivities, activeSubagentActivityId),
    [activeSubagentActivityId, threadActivities],
  );
  const focusedSubagentEntry = activeSubagentActivityId ? (workLogEntries[0] ?? null) : null;
  ...
  const timelineEntries = useMemo(
    () =>
      deriveTimelineEntries(
        timelineMessages,
        activeSubagentActivityId ? [] : (activeThread?.proposedPlans ?? []),
        workLogEntries,
      ),
    [activeSubagentActivityId, activeThread?.proposedPlans, timelineMessages, workLogEntries],
  );
  ...
  const onOpenSubagent = useCallback(
    (activityId: string) => {
      void navigate({
        to: "/$threadId",
        params: { threadId },
        search: { subagentActivityId: activityId },
      });
    },
    [navigate, threadId],
  );
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      >
        <ChatHeader
          ...
          activeThreadTitle={focusedSubagentEntry?.toolTitle ?? activeThread.title}
      ...
      <div className="flex min-h-0 min-w-0 flex-1">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div className="relative flex min-h-0 flex-1 flex-col">
            ...
            >
              {focusedSubagentEntry && (
                <div className="mb-3 flex items-start justify-between gap-3 rounded-xl border border-border/45 bg-card/25 px-3 py-2 text-sm">
                  <div className="min-w-0">
                    <p className="font-medium text-foreground">
                      {focusedSubagentEntry.toolTitle ?? "Subagent task"}
                    </p>
                    <p className="truncate text-muted-foreground text-xs">
                      {focusedSubagentEntry.detail ?? "Focused delegated task view"}
                    </p>
                  </div>
                  <Button
                    type="button"
                    ...
                    onClick={() => {
                      void navigate({
                        to: "/$threadId",
                        params: { threadId },
                        search: (previous) => stripSubagentSearchParams(previous),
                      });
                    }}
                  >
                    Back to thread
              ...
              <MessagesTimeline
                key={`${activeThread.id}:${activeSubagentActivityId ?? "thread"}`}
          ...
          {/* Input bar */}
          {focusedSubagentEntry ? (
            <div className="px-3 pb-3 pt-1.5 text-muted-foreground text-xs sm:px-5 sm:pt-2 sm:pb-4">
              This delegated task view is read-only. Return to the parent thread to continue the
              conversation.
            </div>
          ) : (
          ...
          {isGitRepo && !focusedSubagentEntry && (
            <BranchToolbar
              threadId={activeThread.id}
